// Test: C++17 std::variant, std::visit, std::any, std::optional chaining
#include <variant>
#include <any>
#include <optional>
#include <cassert>
...
int main() {
    ...
    // optional chaining via nested value_or (and_then is C++23)
    auto inner = safe_div(100, 4);
    auto result = inner ? safe_div(inner.value(), 5).value_or(0) : 0;
    assert(result == 5);
